            ...
            <SplitPane :min-percent="10" :default-percent="33" split="vertical">
              ...
              <template slot="paneR">
                <SplitPane :min-percent="10" :default-percent="50" split="vertical">
                  ...
                  <template slot="paneR">
                    <el-button size="small" @click="addItemInForm">
...
export default {
  data() {
    return {
      ...
      banner: {
        title: "DICOM规则管理",
        subTitle: "在这里可以进行DICOM规则的添加,删除,更改等功能",
      },
      columns: [
        {
          title: "序号",
  ...
  computed: {
    data: function () {
      ...
      for (x in this.mid_data) {
        this.mid_data[x]["number_value"] = constant.characterDeal(
          this.mid_data[x]["value"]
        );
        this.mid_data[x]["char_value"] = constant.charDeal(
          this.mid_data[x]["value"]
        );
    ...
    input2: function () {
      if (this.radio != 2) this.inputStr2 = "";
      return this.radio != 2;
    },
    input3: function () {
      if (this.radio != 3) this.inputStr3 = "";
      return this.radio != 3;
    },
  },
  mounted() {
        ...
        if (row.radio === 4) {
          ...
          newRow["value"] =
            row.radio === 1
              ? row.value
              : row.radio === 2
              ? constant.changeString(row.char_value)
              : constant.changeIntString(row.number_value);
          updateDICOMRule({
            ...newRow,
          })
            .then((res) => {
    ...
    addItemInForm() {
      ...
      newRow["value"] =
        this.radio === 1
          ? this.inputStr1
          : this.radio === 2
          ? constant.changeString(this.inputStr2)
          : constant.changeIntString(this.inputStr3);
      addDICOMRule({
        ...newRow,
      })
        .then((res) => {
