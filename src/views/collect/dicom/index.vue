  ...
  <d2-container :type="containerType" :scroll-delay="scrollDelay" better-scroll>
      <template slot="header">
        <div style="margin: -16px;">
          <SplitPane :min-percent='10' :default-percent='20' split="vertical">
            <template slot="paneL">
            ...
            <template slot="paneR">
              <SplitPane :min-percent='10' :default-percent='25' split="vertical">
                <template slot="paneL">
                  ...
                  <el-checkbox style="margin: 10px;" slot="paneR" v-model="checkbox2">{{radioString}}</el-checkbox>
                  </SplitPane>
                </template>
                <template slot="paneR">
  ...
  methods: {
    ...
    searchItemInForm(){
      ...
      if (this.checkbox2 && this.inputStr2 === ''){
        ...
        // 4 :port  :8080
        let re = new RegExp("(^((2(5[0-5]|[0-4]\\d))|[0-1]?\\d{1,2})(\\.((2(5[0-5]|[0-4]\\d))|[0-1]?\\d{1,2})){3}$)|(^((2(5[0-5]|[0-4]\\d))|[0-1]?\\d{1,2})(\\.((2(5[0-5]|[0-4]\\d))|[0-1]?\\d{1,2})){3}:[0-9]{0,5}$)|(^:[0-9]{1,5}$)")
        if(!re.test(this.inputStr2)){
          this.$notify({
            'title':'IP地址和端口号输入不正确',
            'message':'<strong>主要有以下几种形式:</strong><br><i>ip    127.0.0.1</i><br><i>ip:     127.0.0.1:</i><br><i>ip:port     127.0.0.1:8080</i><br><i>:port     :8080</i>',
            type:'warining',
        ...
        if(this.inputStr2.indexOf(':') == -1){
          ...
          if (this.inputStr2.indexOf(':') === this.inputStr2.length - 1){
            newRow['ip'] = this.inputStr2.substring(0, this.inputStr2.length - 1)
